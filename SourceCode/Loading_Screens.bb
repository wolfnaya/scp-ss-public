...
Function DrawLoadingText%(x%, y%, AlignX% = False, AlignY% = False)
	Local ltc.LoadingTextColor
...
Function DrawLoading(percent%, shortloading=False, customloadingscreen$="", Assets$="INITIALIZATION")
	CatchErrors("Function DrawLoading (" + percent + ", " + shortloading + ", " + customloadingscreen + ")")
	Local ls.LoadingScreens
	Local x%, y%, width%, height%
	Local strtemp$, i%,ltc.LoadingTextColor
						...
						If ls\ID = temp Then
							If (Not ls\noAuto) Then
								For i = 0 To (ls\imgamount-1)
									If ls\img[i] = 0 Then ls\img[i] = LoadImage_Strict("GFX\Loadingscreens\"+ls\imgpath[i])
	...
	Repeat
		...
		If (Not shortloading) Then
			If percent <> 100 Then
				Color(255, 255, 255)
				Text(Viewport_Center_X, opt\GraphicHeight - (35 * MenuScale), "LOADING ASSETS: " + Assets, True, True)
				
				FlushKeys()
				FlushMouse()
			Else
				DrawLoadingText(Viewport_Center_X, opt\GraphicHeight - (35 * MenuScale), True, True)
			EndIf
		EndIf
