...
Type Mission
	...
	Field startroom$
	Field musicname$
	Field InvDataName$[MaxItemArray]
	Field InvDataTempName$[MaxItemArray]
	Field TaskAmount%
	Field TaskText$[5]
	Field GunInHand%
...
Function InitMission(id%)
	...
	Select id
		...
		Case 1
			...
			currmi\InvDataName[0] = "Silenced H&K USP"
			currmi\InvDataTempName[0] = "usp"
	...
	For y = MapHeight - 1 To 1 Step - 1
		...
		For x = 1 To MapWidth - 2
			If MapTemp[x * MapWidth + y] > 0 Then
				If gc\CurrZone = HCZ Then temp=2 Else temp=0
...
Function UpdateMissionEnding()
	...
	If KillTimer < 0
		...
		ElseIf mi_I\EndingTimer > 0 And mi_I\EndingTimer < 70*8
			mi_I\EndingTimer = mi_I\EndingTimer + fps\Factor[0]
			EntityAlpha mi_I\FadeEntity,Min((mi_I\EndingTimer*0.01),1.0)
		Else
			mi_I\IsEnding = True
		EndIf
...
Function DrawMissionEnding()
	...
	If gc\CurrGamemode<>1 Then Return
