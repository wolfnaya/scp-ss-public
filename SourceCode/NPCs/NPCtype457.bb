...
Function UpdateNPCtype457(n.NPCs)
	...
	If (Not n\IsDead)
			...
			Case SCP457_STATE_ATTACK ;Attacking
				...
				If n\LastSeen = 0.0 Then
					If EntityDistanceSquared(Collider,n\Collider) < PowTwo(2.0) Then
						If psp\Health > 0 And (Not hds\Wearing) Then
		...
		If dist < PowTwo(GetCameraFogRangeFar(Camera)) Then
			If Rand(3)=1 Then
				CreateParticle(EntityX(n\Collider), EntityY(n\Collider) - 0.25, EntityZ(n\Collider), 13, 0.75/(1+(n\State=SCP457_STATE_STUNNED)), -1.0, 25)
		...
		n\State2 = Max((n\State2 - (fps\Factor[0] / 150.0)),1.0) ;Use it as an "Aggression" state for SCP-457
	Else
		n\State4 = n\State4 + fps\Factor[0]
		If n\State4 > 0 And n\State4 < 70*10 Then
			If Rand(2)=1 Then
