...
Function UpdateNPCtype049(n.NPCs)
		...
		Select n\State
			...
			Case SCP049_LOOKING ;looking around before getting active
				...
				Else
					...
					If n\Frame>268.9 Then n\State = SCP049_ACTIVE
				EndIf
				;[End Block]
						...
						If PlayerSeeAble%=True Lor n\State2>0 Then ;Player is visible for 049's sight - attacking
							...
							If dist < PowTwo(0.5) Then
								If wbi\Hazmat>0 Then
									BlurTimer = BlurTimer+fps\Factor[0]*2.5
									If BlurTimer>250 And BlurTimer-fps\Factor[0]*2.5 <= 250 And n\PrevState<>3 Then
									...
									ElseIf BlurTimer => 500
										For i = 0 To MaxItemAmount-1
											If Inventory[i]<>Null Then
												If Instr(Inventory[i]\itemtemplate\tempname,"hazmatsuit") And wbi\Hazmat<3 Then
								...
								ElseIf I_1033RU\Using > 0 And I_1033RU\HP > 0 Then
									BlurTimer = BlurTimer+fps\Factor[0]*2.5
									If BlurTimer > 500 And BlurTimer-fps\Factor[0]*2.5 =< 500 Then
								...
								Else
									...
									If (Not GodMode) Then
										If PlayerRoom\RoomTemplate\Name$ = "cont_049"
											m_msg\DeathTxt = GetLocalString("Singleplayer","scp049_death_1")
											For e.Events = Each Events
												If e\EventName = "cont_049" Then e\EventState[0]=-1 : Exit
											Next
										Else
											m_msg\DeathTxt = GetLocalString("Singleplayer","scp049_death_2")
											Kill()
										EndIf
	...
	n\LastSeen = Max(n\LastSeen-fps\Factor[0],0)
	n\State2 = Max(n\State2-fps\Factor[0],0)
	n\HP = Min(n\HP+fps\Factor[0]*0.5,100)
