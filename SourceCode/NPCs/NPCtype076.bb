...
Function UpdateNPCtype076(n.NPCs)
	...
	Local agitateHealth% = n\HP/2.0
	
	If (Not n\IsDead) Then
		
		If (Not n\State = SCP076_EVENT_WANDER) Then
			If opt\BMSBossMusic Then
				ShouldPlay = MUS_BMS_076_FIGHT; ~ Case Music
			Else
				ShouldPlay = MUS_076_FIGHT; ~ Case Music
			EndIf
		EndIf
		
		If (Not n\State = SCP076_INTRO) Then
			If EntityDistanceSquared(Collider, n\Collider)>PowTwo(11.5) And (Not n\State = SCP076_INTRO Lor n\State = SCP076_EVENT_WANDER) Then
				n\State = SCP076_WANDER
				If opt\BMSBossMusic Then
					ShouldPlay = MUS_BMS_076_PAUSE; ~ Pause Music
				Else
		...
		If n\State = SCP076_WANDER And (I_268\Using > 0 And I_268\Timer > 0.0 )Then
			If opt\BMSBossMusic Then
				ShouldPlay = MUS_BMS_076_PAUSE; ~ Pause Music
			Else
				ShouldPlay = MUS_076_PAUSE; ~ Pause Music
		...
		Select n\State
			...
			Case SCP076_DETECTED
				...
				If n\LastSeen > 0 Then
					n\State = Rand(SCP076_RUN,SCP076_RUN_2)
				Else
					n\CurrSpeed = CurveValue(0.015 + (0.005*(n\HP<=agitateHealth)), n\CurrSpeed, 5.0)
					AnimateNPC(n, 236, 260, n\CurrSpeed * 18)
				EndIf
			...
			Case SCP076_RUN
				...
				n\CurrSpeed = CurveValue(n\Speed + (0.005*(n\HP<=agitateHealth)), n\CurrSpeed, 5.0)
			...
			Case SCP076_RUN_2
				...
				n\CurrSpeed = CurveValue(n\Speed*1.8 + (0.005*(n\HP<=agitateHealth)), n\CurrSpeed, 5.0)
				AnimateNPC(n, 301, 319, n\CurrSpeed * 20)
							...
							If opt\BMSBossMusic Then
								If psp\Health =< 0 Then
									If opt\MusicVol > 0 Then
										PlaySound_Strict(LoadTempSound("SFX\Music\Boss_Fights\BMS\076_End.ogg"))
									EndIf
									ShouldPlay = MUS_NULL
								EndIf
							EndIf
			...
			Case SCP076_STUNNED
				...
				If prevFrame < 629.5 And n\Frame >= 629.5 Lor prevFrame = 630.0 And n\Frame = 630.0 Then
					...
					If n\State <> SCP076_DETECTED Then
						n\State = SCP076_WANDER
						n\GotHit = False
					EndIf
				EndIf
		...
		If n\State = SCP076_WANDER Lor n\State = SCP076_RUN Lor n\State = SCP076_RUN_2 Lor n\LastSeen <= 0 Then
			If n\CurrSpeed > 0.01 Then
		...
		If n\State > SCP076_FREEZE And n\State < SCP076_ATTACK Then
		...
		If n\LastSeen <= 0.0 Then
			If n\State = SCP076_WANDER Lor n\State = SCP076_DETECTED Then
				If n\GotHit Then
		...
		If n\HP <= 0 Then
			...
			GiveAchievement(Achv076) ; ~ Achievement
		EndIf
	Else
...
Function UpdateNPCtype076MP(n.NPCs)
	...
	If (Not n\IsDead) Then
		...
		If n\State = SCP076_WANDER Lor n\LastSeen <= 0 Then
			If n\CurrSpeed > 0.01 Then
				If prevFrame < 244 And AnimTime(n\obj)=>244 Then
					sfxstep = GetStepSound(n\Collider,n\CollRadius)
					PlaySound2(StepSFX(sfxstep,0,Rand(0,7)),Camera, n\Collider, 8.0, Rnd(0.3,0.5))
