...
Function CreateNPCtype096(n.NPCs)
	...
	EntityTexture(n\obj2, OverlayTexture[10])
	HideEntity(n\obj2)
...
Function UpdateNPCtype096(n.NPCs)
	...
	If wbi\SCRAMBLE > 0 And Dist < PowTwo(CameraFogFar * LightVolume) And (Angle < 135.0 Lor Angle > 225.0) And EntityVisible(Camera, n\obj2) Then
	...
	Select n\State
			...
			If Dist < 64.0 Then
				...
				If (I_268\Using <> 3) Then
					If Dist < PowTwo(CameraFogFar * LightVolume) Then
						If (Not wbi\SCRAMBLE) And (Angle < 135.0 Lor Angle > 225.0) And (EntityVisible(Camera, n\obj2) And EntityInView(n\obj2, Camera)) Then
		...
		Case 4.0
			...
			If I_268\Using = 3 And n\Target = Null Then n\State = 5.0
