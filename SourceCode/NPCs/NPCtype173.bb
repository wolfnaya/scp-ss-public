		...
		If n\Idle < SCP173_BOXED Then
			If n\Idle = SCP173_ACTIVE Then
				...
				If move=False Then
					...
					n\State = Max(0, n\State - fps\Factor[0] / 20)
				Else 
...
;							EndIf
							...
							If n2\NPCtype = NPCTypePlotNTF And n2\HP > 0 Then
								...
								EndIf
							EndIf
							If n2\NPCtype = NPCTypeCITest And n2\HP > 0 Then
								dist2 = EntityDistanceSquared(n\Collider, n2\Collider)
								If dist2 < stored_dist Then
									stored_dist = dist2
									n\Target = n2
						...
						If temp And n\Target = Null Then
							...
							If dist < 0.65 Then
								If KillTimer >= 0 And (Not GodMode) Then
									...
									If (Not hds\Wearing And hds\Health > 50) Then
						...
						Else ;player is not visible -> move to the location where he was last seen
							If n\EnemyX <> 0 Then
								...
								If DistanceSquared(EntityX(n\Collider), n\EnemyX, EntityZ(n\Collider), n\EnemyZ) > PowTwo(0.5) Then
									...
									MoveEntity(n\Collider, 0, 0, n\Speed * fps\Factor[0])
									If Rand(500) = 1 Then
										n\EnemyX = 0
										n\EnemyY = 0
										n\EnemyZ = 0
		...
		ElseIf n\Idle = SCP173_BOXED Lor n\Idle = SCP173_CONTAINED Then
			If n\Idle = SCP173_BOXED Then
				...
				If EntityDistanceSquared(n\Collider, Collider) < 1.0 ;PowTwo(1) = 1 / 1*1 = 1 Then
					MoveEntity n\Collider, 0, 0, -0.016 * fps\Factor[0] * Max(Min((EntityDistanceSquared(n\Collider, Collider) * PowTwo(2) - 1.0) * 0.5, 1.0), -0.5)
				Else
					MoveEntity n\Collider, 0, 0, 0.016 * fps\Factor[0] * Max(Min((EntityDistanceSquared(n\Collider, Collider) * PowTwo(2) - 1.0) * 0.5, 1.0), -0.5)
				EndIf
				n\GravityMult = 1.0
