...
; ~ "Normal" zombies
Function CreateNPCtype049_2MP(n.NPCs,model%=-1)
	Local temp#
...
Function UpdateNPCtype049_2MP(n.NPCs)
			...
			Case MPZ_STATE_WANDER ; ~ Wandering around
				...
				If n\PathStatus=1 Then
					...
					If n\PathStatus=1 Then
						...
						If n\NPCtype = NPCType049_2MP Then
			...
			Else
				If prevFrame < 392.5 And n\Frame => 392.5 Lor prevFrame < 423.5 And n\Frame => 423.5 Then
					...
					Select Random
						...
						Case 2
							it = CreateItem("Heavy Ballistic Vest","heavyvest",EntityX(bone%,True),EntityY(bone%,True),EntityZ(bone%,True))
						Case 3	
							it = CreateItem("Big Ammo Crate","bigammocrate",EntityX(bone%,True),EntityY(bone%,True),EntityZ(bone%,True))
					End Select
					EntityType it\Collider, HIT_ITEM
					it\Dropped = 1
				EndIf
