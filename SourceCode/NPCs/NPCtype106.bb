		...
		If (Not n\Idle) And spawn106%
			If n\State <= 0 Then ;attacking	
				...
				Else
					...
					If PlayerRoom\RoomTemplate\Name <> "gate_a_topside" And PlayerRoom\RoomTemplate\Name <> "area_106_escape" And PlayerRoom\RoomTemplate\Name <> "room3_ct" Then 
						ShouldPlay = (Rand(MUS_CHASE_106,MUS_CHASE_106_2))
					ElseIf PlayerRoom\RoomTemplate\Name = "area_106_escape" Then
					...
					If dist > 0.8 Then
						If (dist > 25.0 Lor PlayerRoom\RoomTemplate\Name = "pocketdimension" Lor Visible Lor n\PathStatus <> 1) And PlayerRoom\RoomTemplate\Name <> "gate_a_topside" And ((I_268\Using = 0 Lor I_268\Timer =< 0.0)) Then 
							...
							n\PathTimer = Max(n\PathTimer-fps\Factor[0],0)
				...
				For n2 = Each NPCs
					If n2\NPCtype = NPCtypeD2 And n2\HP > 0 Then
						...
						EndIf
					EndIf
					If n2\NPCtype = NPCtypeCI And n2\HP > 0 Then
						dist2 = EntityDistanceSquared(n\Collider, n2\Collider)
						If dist2 < stored_dist Then
							stored_dist = dist2
							n\Target = n2
						...
						EndIf
					EndIf
					If n2\NPCtype = NPCtypeNTF And n2\HP > 0 Then
						dist2 = EntityDistanceSquared(n\Collider, n2\Collider)
						If dist2 < stored_dist Then
							stored_dist = dist2
							n\Target = n2
						...
						EndIf
					EndIf
					If n2\NPCtype = NPCTypePlotNTF And n2\HP > 0 Then
						dist2 = EntityDistanceSquared(n\Collider, n2\Collider)
						If dist2 < stored_dist Then
							stored_dist = dist2
							n\Target = n2
						...
						EndIf
					EndIf
					If n2\NPCtype = NPCTypeNTFEnemy And n2\HP > 0 Then
						dist2 = EntityDistanceSquared(n\Collider, n2\Collider)
						If dist2 < stored_dist Then
							stored_dist = dist2
							n\Target = n2
				...
				If (Not PlayerRoom\RoomTemplate\DisableDecals) Then
					If PlayerRoom\RoomTemplate\Name <> "gate_a_topside"
						If (SelectedDifficulty\AggressiveNPCs) Then
							n\State=n\State-fps\Factor[0]*2
						Else
							n\State=n\State-fps\Factor[0]
						EndIf
					EndIf
