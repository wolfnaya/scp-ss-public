...
Function UpdatePlayerGun(playerID%)
	...
	Local DeployTime#,Accuracy#,DamageOnEntity%,RateOfFire#,MaxAmmo%,ReloadTime#,ReloadNotFullTime#,ReloadStartTime#
	...
	If Players[playerID]\CurrHP > 0 Then
		...
		For g = Each Guns
			If g\ID = Players[playerID]\WeaponInSlot[Players[playerID]\SelectedSlot] Then
				...
				ReloadTime = g\Reload_Empty_Time
				ReloadNotFullTime = g\Reload_Time
		...
		If Players[playerID]\DeployState < DeployTime Then
			Players[playerID]\DeployState = Players[playerID]\DeployState + fps\Factor[0]
		Else
			If Players[playerID]\IsPlayerSprinting And GunType <> GUNTYPE_MELEE And GunType <> GUNTYPE_GRENADE Then
				Players[playerID]\PressMouse1 = False
			EndIf
			
			If GunType = GUNTYPE_SHOTGUN Then; Lor GunType = GUNTYPE_AUTO_SHOTGUN
				...
				If Players[playerID]\PressReload Then
					Players[playerID]\ReloadState = fps\Factor[0]
				EndIf
				
				If Players[playerID]\ReloadState > 0.0 And Players[playerID]\ReloadState < ReloadStartTime Then
					Players[playerID]\ShootState = 0.0
					Players[playerID]\ReloadState = Players[playerID]\ReloadState + fps\Factor[0]
			...
			ElseIf GunType = GUNTYPE_MELEE Then
				...
				If Players[playerID]\ShootState > 0.0 And Players[playerID]\ShootState < RateOfFire Then
					Players[playerID]\ShootState = Players[playerID]\ShootState + fps\Factor[0]
					If Players[playerID]\ShootState >= AttackDelay And Players[playerID]\ShootState <= AttackDelay+fps\Factor[0] Then
						For i=1 To ShootAmount
				...
				Else
					Players[playerID]\ShootState = 0.0
				EndIf
			ElseIf GunType = GUNTYPE_GRENADE Then
				Players[playerID]\PressReload = False
				...
				If Players[playerID]\ShootState > 0.0 And Players[playerID]\ShootState < RateOfFire Then
					...
					If Players[playerID]\ShootState >= AttackDelay And Players[playerID]\ShootState <= AttackDelay+fps\Factor[0] Then
						For i=1 To ShootAmount
							CreateGrenadeMP(playerID,EntityX(Camera),EntityY(Camera),EntityZ(Camera), EntityPitch(Camera), EntityYaw(Camera), g)
						Next
					EndIf
				Else
					Players[playerID]\ShootState = 0.0
			...
			Else
				...
				ElseIf Players[playerID]\ReloadState > 0.0 And Players[playerID]\ReloadState <= ReloadTime And Players[playerID]\Ammo[Players[playerID]\SelectedSlot] = 0 Then
					...
					If playerID = mp_I\PlayerID Then
						mp_I\LocalAmmo = 0
					EndIf
				ElseIf Players[playerID]\ReloadState > 0.0 And Players[playerID]\ReloadState < ReloadNotFullTime And Players[playerID]\Ammo[Players[playerID]\SelectedSlot] > 0 Then
					Players[playerID]\ShootState = 0.0
					Players[playerID]\ReloadState = Players[playerID]\ReloadState + fps\Factor[0]
					If mp_I\PlayState = GAME_SERVER Then
						If GunType = GUNTYPE_SHOTGUN Then; Lor GunType = GUNTYPE_AUTO_SHOTGUN
				...
				Else
					Players[playerID]\ReloadState = 0.0
					If mp_I\PlayState = GAME_SERVER Then
						pAmmoDelta% = g\MaxCurrAmmo - Players[playerID]\Ammo[Players[playerID]\SelectedSlot]
						Select Players[playerID]\WeaponInSlot[Players[playerID]\SelectedSlot]
							Case 9
	...
	If ent_pick%<>0 Then
		...
		For n.NPCs = Each NPCs
			...
			If currHP <= 0 And prevHP > 0 Then
				...
				If playerID = mp_I\PlayerID Then
					If n\NPCtype = NPCType049_2MP Lor n\NPCtype = NPCtypeGuardZombieMP Then
	...
	ElseIf (Not g_I\GunAnimFLAG) And Players[mp_I\PlayerID]\CurrSpeed<>0.0 And (Not Players[mp_I\PlayerID]\IsPlayerSprinting%) And (Not Players[mp_I\PlayerID]\IronSight)
        If GunPivot_XSide%=0
            If GunPivot_X# > -0.0025
                GunPivot_X# = GunPivot_X# - (0.000075 / (1.0 + (Players[mp_I\PlayerID]\Crouch)) * fps\Factor[0])
                If GunPivot_X# > -0.00125
                    GunPivot_Y# = Min(GunPivot_Y#+(0.000125 / (1.0 + (Players[mp_I\PlayerID]\Crouch)) * fps\Factor[0]),0.001)
                Else
                    GunPivot_Y# = Max(GunPivot_Y#-(0.000125 / (1.0 + (Players[mp_I\PlayerID]\Crouch)) * fps\Factor[0]),0.0)
                EndIf
            Else
                GunPivot_X# = -0.0025
                GunPivot_Y# = 0.0
...
Function CreateGrenadeMP.Grenades(playerid%, x#, y#, z#, pitch#, yaw#, gun.Guns)
	...
	gr\ID = mp_I\CurrGrenadeID
	
	mp_I\CurrGrenadeID = mp_I\CurrGrenadeID + 1
	
	gr\Gun = gun
	gr\Gun\CurrAmmo = 0
	Players[playerid]\WeaponInSlot[Players[playerid]\SelectedSlot] = ""
