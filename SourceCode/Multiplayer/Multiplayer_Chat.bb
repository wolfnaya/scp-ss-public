...
Function AddChatMSG.ChatMSG(first_msg$, playerID%, isServerMSG% = SERVER_MSG_NO, msg_type% = CHATMSG_TYPE_DEFAULT)
	...
	If gc\CurrGamemode <> 3 Then Return
...
Function UpdateChat()
	...
	For cmsg = Each ChatMSG
		If cmsg\Time >= 70*10.0 Then
			Delete cmsg
		Else
			cmsg\Time = cmsg\Time + fps\Factor[0]
		EndIf
	Next
	
	If MenuOpen Lor ConsoleOpen Lor AttachmentOpen Lor IsPlayerListOpen() Lor IsModerationOpen() Lor (Not HUDenabled) Then
		Return
	EndIf
...
Function RenderChat()
	...
	If InLobby() Lor MenuOpen Lor ConsoleOpen Lor AttachmentOpen Lor IsPlayerListOpen() Lor IsModerationOpen() Lor (Not HUDenabled) Then
		Return
	EndIf
