...
Function InitMultiplayer()
	...
	mp_O\BanFile = GetEnv("AppData")+"\scp-ss\server_bans.txt"
	ReadBanList()
	...
	While (ft\accumulator>0.0)
		...
		Else
			...
			If (Not co\Enabled)
				...
				If (Not InLobby()) And (Not MenuOpen) And (Not ConsoleOpen) And (Not AttachmentOpen) And (Not mp_I\ChatOpen) And (Not IsModerationOpen()) And (Not IsInVote()) Then
			...
			Select mp_I\Gamemode\ID
				Case Gamemode_Waves
					If (mp_I\Gamemode\Phase Mod 2)=1 Lor InLobby() Then
						If mp_I\MapInList\Name = "Offices" Then
							ShouldPlay = MUS_MP_IDLE
						Else
							ShouldPlay = MUS_MP_IT_WATCHES
						EndIf
					Else
						If mp_I\MapInList\Name = "Offices" Then
							ShouldPlay = MUS_MP_FIGHT
						Else
							If ShouldPlay < MUS_MP_IT_FOLLOWS Lor ShouldPlay > MUS_MP_CONFLICT_ARISES Then
								ShouldPlay = MUS_MP_IT_FOLLOWS-Rand(0,1)
							EndIf	
						EndIf
					EndIf
		...
		MouseLookServer()
		If (Not IsSpectator(mp_I\PlayerID)) And mpl\HasNTFGasmask Then
			MovePlayerServer()
		...
		UpdateFluLights()
		If (Not IsSpectator(mp_I\PlayerID)) Then
			UpdateGuns()
		EndIf
		UpdatePlayerGunsServer()
		PlayGunSoundsMP()
		AnimateGunsServer()
		If Players[mp_I\PlayerID]\CurrHP > 0 And (Not ConsoleOpen) And (Not AttachmentOpen)  And (Not mp_I\ChatOpen) And (Not MenuOpen) And (Not InLobby()) And (Not IsModerationOpen()) And (Not IsInVote()) Then
		...
		;ecst\CompletedGame = True
		
		If mp_I\PlayState = GAME_SERVER Then
			If mp_I\Gamemode\ID = Gamemode_Gungame Then
				If mp_I\Gamemode\Phase > Gungame_CILost Then
					WonAsRRH = True
				ElseIf mp_I\Gamemode\Phase > Gungame_MTFLost Then
					WonAsSH = True
				EndIf
			EndIf
		EndIf
		
		If WonAsRRH And WonAsSH Then
			GiveAchievement(AchvGungame)
		EndIf
		...
		If KeyHit(KEY_SELECTATTACHMENT) And (Not ConsoleOpen) And (Not mp_I\ChatOpen) And (Not MenuOpen) And (Not InLobby()) And (Not IsModerationOpen()) And (Not IsInVote()) Then
					...
					Select currMSGSync
						...
						Case PACKET_PLAYER
							...
							If Players[TempID]<>Null Then
								...
								; ~ Anti - Cheat system (Bullets)
;								If prevSlot = Players[TempID]\SelectedSlot Then
;									For g = Each Guns
;										If g\ID = Players[TempID]\WeaponInSlot[Players[TempID]\SelectedSlot] Then
;											If (g\GunType <> GUNTYPE_MELEE And g\GunType <> GUNTYPE_GRENADE And g\GunType <> GUNTYPE_SHOTGUN) Then
...
Function RecvDataClient()
	...
	Local temp%
	Local it.Items,hasItem%,deleteitem%,itID%,itX#,itY#,itZ#,itName$,itTempName$,itYaw#,itFound.Items,fb.FuseBox,ge.Generator,dbr.DamageBossRadius,pg.ParticleGen,lg.LeverGen,bg.ButtonGen,g.Guns,gr.Grenades
	Local n.NPCs,deletenpc%,nID%,nX#,nY#,nZ#,nYaw#,ntype%,nFound.NPCs,nHP%,nCurrPlayer%
	Local grID%,grPlayerID,grX#,grY#,grZ#,grPitch#,grYaw#,grFound.Grenades,gID,currWep.Guns
	Local gunAmmo1%,gunAmmo2%,reloadAmmo1%,reloadAmmo2%,sslot%,shootstate#,reloadstate#,deploystate#
	Local nState1#,nState2#,nState3#,nState4#
	...
	;									SpriteViewMode p\obj,1
			...
			Case PACKET_GRENADE
				...
				If temp>0 Then
					For i=1 To temp
						grID = Steam_PullInt()
						grPlayerID = Steam_PullInt()
						grX = Steam_PullFloat()
						grY = Steam_PullFloat()
						grZ = Steam_PullFloat()
						grPitch = Steam_PullFloat()
						grYaw = Steam_PullFloat()
						gID = Steam_PullByte()
						...
						If grFound <> Null Then
							If Abs(EntityX(grFound\obj)-grX) > 0.5 Lor Abs(EntityY(grFound\obj)-grY) > 0.5 Lor Abs(EntityZ(grFound\obj)-grZ) > 0.5 Then
								PositionEntity grFound\obj,grX,grY,grZ
								ResetEntity grFound\obj
							Else
								PositionEntity grFound\obj,CurveValue(grX,EntityX(grFound\obj),10.0),CurveValue(grY,EntityY(grFound\obj),5.0),CurveValue(grZ,EntityZ(grFound\obj),10.0)
							EndIf
							RotateEntity grFound\obj,CurveAngle(grPitch,EntityPitch(grFound\obj),10.0),CurveAngle(grYaw,EntityYaw(grFound\obj),10.0),0
						Else
							For g = Each Guns
								If g\ID = gID Then
									currWep = g
								...
								EndIf
							Next
							gr = CreateGrenadeMP(grPlayerID,grX,grY,grZ,grPitch,grYaw,currWep)
							gr\ID = grID
						EndIf
					Next
				EndIf
			...
			Case PACKET_GAMEMODEUTIL
				...
				ElseIf mp_I\Gamemode\ID = Gamemode_Gungame Then
					If prevPhase > Gungame_Game And mp_I\Gamemode\Phase = Gungame_GameStart Then
						If Players[mp_I\PlayerID]\Team = Team_CI Then
							mp_I\Map\CurrChunk = mp_I\MapInList\CISpawn-(mp_I\MapInList\ChunkStart-1)
						Else
							mp_I\Map\CurrChunk = mp_I\MapInList\NTFSpawn-(mp_I\MapInList\ChunkStart-1)
						EndIf
					EndIf
					If mp_I\Gamemode\Phase = Gungame_Game Then
						If mp_I\Gamemode\PhaseTimer > 0 Then
							CountdownBeep(mp_I\Gamemode\PhaseTimer, 3)
						EndIf	
					EndIf
...
Function SyncServer()
	...
	Local g.Guns,ch.ChatMSG,fb.FuseBox,ge.Generator,dbr.DamageBossRadius,pg.ParticleGen,lg.LeverGen,bg.ButtonGen,gr.Grenades
	...
	For i=1 To (mp_I\MaxPlayers-1)
		If Players[i]<>Null
			If Players[i]\FinishedLoading Then
				...
				;Grenades syncing (package)
				;[Block]
				Steam_PushByte(PACKET_GRENADE)
				temp = 0
				...
				For gr = Each Grenades
					Steam_PushInt(gr\ID)
					Steam_PushInt(gr\PlayerID)
					Steam_PushFloat(EntityX(gr\obj))
					Steam_PushFloat(EntityY(gr\obj))
					Steam_PushFloat(EntityZ(gr\obj))
					Steam_PushFloat(EntityPitch(gr\obj))
					Steam_PushFloat(EntityYaw(gr\obj))
					Steam_PushByte(gr\Gun\ID)
				Next
				Steam_SendPacketToUser(Players[i]\SteamIDUpper, Players[i]\SteamIDLower, k_EP2PSendUnreliableNoDelay)
				;[End Block]
	...
	If mp_O\LocalServer=False Then
		mp_I\ServerListUpdateTimer = mp_I\ServerListUpdateTimer + fps\Factor[0]
...
;	SendUDPMsg(mp_I\Server,Players[0]\IP,Players[0]\Port)
	...
	If mp_I\PingTimer > 70.0 Then
		...
		mp_I\PingTimer = 0.0
	EndIf
	mp_I\PingTimer = mp_I\PingTimer + fps\Factor[0]
