...
; ~ Gungame constants
Const Gungame_GameStart = 0
Const Gungame_Game = 1
Const Gungame_MTFLost = 2
Const Gungame_CILost = 3
Const Gungame_TeamSwitch = 4
...
Function LoadMPMaps()
	...
	dirPath = "GFX\map\multiplayer\maps"
	dir = ReadDir(dirPath)
	Repeat 
		file$=NextFile$(dir)
		If file$="" Then Exit
		If FileType(dirPath+"\"+file$) = 2 Then
			If file <> "." And file <> ".." Then
				If (FileType(dirPath+"\"+file+"\config.ini") > 0) Then
					...
					Else
						mfl\Image = LoadImage_Strict("GFX\menu\multiplayer\map_preview_unknown.png")
					EndIf
...
Function CreateMPGameModes()
	...
	mgm\name = "Waves"
	mgm\img[0] = LoadImage_Strict("GFX\menu\multiplayer\skull_logo.png")
	MidHandle mgm\img[0]
	...
	ResizeImage(mgm\img[Team_CI-1], 230 * MenuScale, 230 * MenuScale)
	MaskImage mgm\img[Team_CI-1],255,0,255
	mgm\Image = LoadImage_Strict("GFX\menu\multiplayer\gamemode_preview_gungame.png")
	mgm\Image = ResizeImage2(mgm\Image, 384 * MenuScale, 192 * MenuScale)
	mgm\MaxPlayersAllowed = MaxPlayers
...
;							Local g.Guns, j%, sf%, b%, t1%, name$, tex%
;							
...
;							DeleteSingleTextureEntryFromCache tex
						...
						Case Waves_StartGame
							...
							Else
								mp_I\Gamemode\PhaseTimer = mp_I\Gamemode\PhaseTimer - fps\Factor[0]
							...
							Else	
								For i = 0 To (mp_I\MaxPlayers-1)
									If Players[i]<>Null Then
										...
										EndIf
										SetupTeam(i)
										Players[i]\WeaponInSlot[SLOT_PRIMARY] = 0
										Players[i]\WeaponInSlot[SLOT_SECONDARY] = 5
										Players[i]\WeaponInSlot[SLOT_MELEE] = 1
				...
				Case Gamemode_Gungame
					...
					Select mp_I\Gamemode\Phase
						Case Gungame_GameStart
							...
							mp_I\Gamemode\DisableMovement = True
							mp_I\Gamemode\PhaseTimer = 70*5
							mp_I\Gamemode\Phase = Gungame_Game
						Case Gungame_MTFLost, Gungame_CILost
							If mp_I\Gamemode\PhaseTimer > 0 Then
								mp_I\Gamemode\PhaseTimer = mp_I\Gamemode\PhaseTimer - fps\Factor[0]
							Else
								If mp_I\Gamemode\TeamSwitched = False And (mp_I\Gamemode\RoundWins[Team_MTF-1]+mp_I\Gamemode\RoundWins[Team_CI-1]) = 15 Then
									mp_I\Gamemode\Phase = Gungame_TeamSwitch
									mp_I\Gamemode\PhaseTimer = 70*10
									mp_I\Gamemode\DisableMovement = True
								Else
									mp_I\Gamemode\Phase = Gungame_GameStart
								EndIf
								For it = Each Items
									RemoveItem(it)
								Next
							EndIf
						Case Gungame_TeamSwitch
							If mp_I\Gamemode\PhaseTimer > 0 Then
								mp_I\Gamemode\PhaseTimer = mp_I\Gamemode\PhaseTimer - fps\Factor[0]
							Else	
								For i = 0 To (mp_I\MaxPlayers-1)
									...
									EndIf
								Next
								swapValue% = mp_I\Gamemode\RoundWins[Team_CI-1]
								mp_I\Gamemode\RoundWins[Team_CI-1] = mp_I\Gamemode\RoundWins[Team_MTF-1]
								mp_I\Gamemode\RoundWins[Team_MTF-1] = swapValue
								mp_I\Gamemode\TeamSwitched = True
								mp_I\Gamemode\Phase = Gungame_GameStart
						...
						Case Gungame_Game
							...
							Else
								...
								If maxPeopleOnTeamMTF > 0 And peopleOnTeamMTFDead = maxPeopleOnTeamMTF Then
									roundEnd = True
									mp_I\Gamemode\Phase = Gungame_MTFLost
									mp_I\Gamemode\RoundWins[Team_CI-1] = mp_I\Gamemode\RoundWins[Team_CI-1] + 1
								EndIf
								...
								If maxPeopleOnTeamCI > 0 And peopleOnTeamCIDead = maxPeopleOnTeamCI Then
									roundEnd = True
									mp_I\Gamemode\Phase = Gungame_CILost
									mp_I\Gamemode\RoundWins[Team_MTF-1] = mp_I\Gamemode\RoundWins[Team_MTF-1] + 1
								EndIf
...
Function UpdateGenerators()
	...
	For ge = Each Generator
		If ge\progress > 0.0 Then
			ge\progress = Min(ge\progress + fps\Factor[0], GEN_CHARGE_TIME)
...
Function UpdateDamageBossRadius()
	...
	For dbr = Each DamageBossRadius
		If dbr\timer > 0.0 Then
			dbr\timer = dbr\timer + fps\Factor[0]
			For pg = Each ParticleGen
				If pg\id = dbr\id Then
