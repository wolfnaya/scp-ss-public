...
Function DrawGUIMP()
	...
	If DrawHandIcon Then DrawImage(PlayerIcons[3], opt\GraphicWidth / 2 - 32, opt\GraphicHeight / 2 - 32)
	For i = 0 To 3
	...
	If HUDenabled And (Not InLobby()) And Players[playerid]\Team > Team_Unknown Then ;CHECK FOR IMPLEMENTATION
		...
		If mpl\SlotsDisplayTimer > 0.0 Then
			For i = 0 To MaxSlots-1
				...
				EndIf
				If Players[mp_I\PlayerID]\WeaponInSlot[i]<>0 Then
					For g = Each Guns
				...
				If mp_I\Gamemode\Phase > Deathmatch_Game Then
					...
					Select mp_I\Gamemode\Phase
						Case Deathmatch_MTFLost
							Text x,y,GetLocalString("Multiplayer", "deathmatch_cwbin"),True,True
...
Function UpdateGUIMP()
	...
	If Players[mp_I\PlayerID]\CurrHP > 0.0 Then
		If (Not AttachmentOpen) Then
			For i = 0 To MaxSlots-1
				If KeyHit(i+2) Then
					If (Not InLobby()) And (Not ConsoleOpen) And (Not AttachmentOpen) And (Not mp_I\ChatOpen) Then
						If Players[mp_I\PlayerID]\WeaponInSlot[i]<>0 Then
							If i<>Players[mp_I\PlayerID]\SelectedSlot Then
...
;	EndIf
	...
	If MenuOpen Then
		...
		Else
			SetFont fo\ConsoleFont
		EndIf
		Text 20, opt\GraphicHeight-70, GetLocalString("Serverlist", "username")+mp_O\PlayerName
		Text 20, opt\GraphicHeight-50, GetLocalString("Serverlist", "server_name")+mp_I\ServerName
...
Function DrawPlayerList()
	...
	If IsPlayerListOpen() Then
		...
		y = height * 4
		If mp_I\Gamemode\ID <> Gamemode_Deathmatch And mp_I\Gamemode\ID <> Gamemode_Gungame Then
			y = y + height * 4
		EndIf
		...
		For i = 0 To ((MaxPlayerTeams - 1) And (mp_I\Gamemode\ID = Gamemode_Deathmatch) Lor (mp_I\Gamemode\ID = Gamemode_Gungame))
			SetFont fo\Font[Font_Default_Medium]
			y = y + height * 2
			DrawFrame(x, y, width, height * 1.5)
...
Function UpdatePlayerList()
	...
	If IsPlayerListOpen() Then
		...
		For i = 0 To ((MaxPlayerTeams - 1) And (mp_I\Gamemode\ID = Gamemode_Gungame))
			y = y + height * 2
