...
Function CreatePlayer(playerID%)
	...
	If mp_I\PlayState = GAME_SERVER Then
		...
		Players[playerID]\WeaponInSlot[Players[playerID]\SelectedSlot] = 3
		Players[playerID]\WeaponInSlot[SLOT_MELEE] = 1
...
Function MovePlayerServer()
	...
	If (Not MenuOpen) And (Not ConsoleOpen) And (Not AttachmentOpen) And (Not InLobby()) And (Not mp_I\ChatOpen) And Players[mp_I\PlayerID]\CurrHP>0 And (Not mp_I\Gamemode\DisableMovement) And (Not IsModerationOpen()) And (Not IsInVote()) Then
		...
		If Players[i]<>Null Then
			If (Not IsSpectator(i)) Then ;CHECK FOR IMPLEMENTATION
				If Players[i]\StaminaEffectTimer > 0 Then
					Players[i]\CurrStamina = Min(Players[i]\CurrStamina + fps\Factor[0], 100.0)
				Else
					If Players[i]\CurrSpeed > 0 Then
						Players[i]\CurrStamina = Min(Players[i]\CurrStamina + 0.15 * fps\Factor[0]/1.25, 100.0)
					Else
						Players[i]\CurrStamina = Min(Players[i]\CurrStamina + 0.15 * fps\Factor[0]*1.25, 100.0)
					EndIf
				EndIf	
				
				Players[i]\StaminaEffectTimer = Max(0,Players[i]\StaminaEffectTimer - fps\Factor[0])
...
Function UpdatePlayerPosition(playerID%)
	...
	If Players[playerID]\walking Then
		...
		If Players[playerID]\Crouch = 0 And (Players[playerID]\PressSprint) And Players[playerID]\CurrStamina > 0.0 And Players[playerID]\IronSight = 0 Then
			...
			Players[playerID]\CurrStamina = Players[playerID]\CurrStamina - fps\Factor[0] * 0.25
	...
	TranslateEntity Players[playerID]\Collider, Cos(angle)*Players[playerID]\CurrSpeed * fps\Factor[0], 0, Sin(angle)*Players[playerID]\CurrSpeed * fps\Factor[0], True
	...
	Else
		Players[playerID]\DropSpeed# = Min(Max(Players[playerID]\DropSpeed - 0.006 * fps\Factor[0], -2.0), 0.0)
	EndIf
	PlayerFallingPickDistance# = 10.0
	
	TranslateEntity Players[playerID]\Collider, 0, Players[playerID]\DropSpeed * fps\Factor[0], 0
...
Function UpdateSpectatorPosition(playerID%)
	...
	;[CONTROLLER]
	If (Not co\Enabled)
		If KeyDown(KEY_DOWN) Then MoveEntity Players[playerID]\Collider, 0, 0, -temp*fps\Factor[0]
		If KeyDown(KEY_UP) Then MoveEntity Players[playerID]\Collider, 0, 0, temp*fps\Factor[0]
		
		If KeyDown(KEY_LEFT) Then MoveEntity Players[playerID]\Collider, -temp*fps\Factor[0], 0, 0
		If KeyDown(KEY_RIGHT) Then MoveEntity Players[playerID]\Collider, temp*fps\Factor[0], 0, 0
	Else
		If GetLeftAnalogStickPitch()<0.0
			MoveEntity Players[playerID]\Collider, 0, 0, -temp*fps\Factor[0]
		EndIf
		If GetLeftAnalogStickPitch()>0.0
			MoveEntity Players[playerID]\Collider, 0, 0, temp*fps\Factor[0]
		EndIf
		If GetLeftAnalogStickYaw(True)<0.0
			MoveEntity Players[playerID]\Collider, -temp*fps\Factor[0], 0, 0
		EndIf
		If GetLeftAnalogStickYaw(True)>0.0
			MoveEntity Players[playerID]\Collider, temp*fps\Factor[0], 0, 0
		EndIf
...
Function MouseLookServer()
	...
	If (Not MenuOpen) And (Not ConsoleOpen) And (Not AttachmentOpen) And (Not InLobby()) And (Not mp_I\ChatOpen) And (mpl\WheelOpened=WHEEL_CLOSED) And (Not IsPlayerListOpen()) And (Not IsModerationOpen()) And (Not IsInVote()) Then ;And (Not Players[mp_I\PlayerID]\Disable) Then
	...
	If (Not MenuOpen) And (Not ConsoleOpen) And (Not AttachmentOpen) And (Not InLobby()) And (Not mp_I\ChatOpen) And (Not IsPlayerListOpen()) And (Not IsModerationOpen()) And (Not IsInVote()) Then
		...
		If Players[i]<>Null Then
			...
			If Players[i]\CurrHP <= 0 Then ;CHECK FOR IMPLEMENTATION
				...
				If i = mp_I\PlayerID Then
					...
					If (Not MenuOpen) And (Not ConsoleOpen) And (Not AttachmentOpen) And (Not InLobby()) And (Not IsModerationOpen()) And (Not IsInVote()) Then
						If MouseHit1 Then
...
Function RespawnPlayer(playerID%)
	...
	Local beforeWeapon% = (Players[playerID]\WeaponInSlot[SLOT_PRIMARY] <> 0 Lor Players[playerID]\SelectedSlot <> SLOT_PRIMARY)
	If Players[playerID]\CurrHP <= 0 Then
		Players[playerID]\CurrHP = 100
		Players[playerID]\CurrKevlar = 100
		Players[playerID]\CurrStamina = 100.0
		...
		Players[playerID]\SelectedSlot = SLOT_SECONDARY
		Players[playerID]\WeaponInSlot[SLOT_PRIMARY] = 0
		Players[playerID]\WeaponInSlot[Players[playerID]\SelectedSlot] = 3
		Players[playerID]\WeaponInSlot[SLOT_MELEE] = 1
		Players[playerID]\Ammo[Players[playerID]\SelectedSlot] = GetWeaponMaxCurrAmmo(Players[playerID]\WeaponInSlot[Players[playerID]\SelectedSlot])
		Players[playerID]\ReloadAmmo[Players[playerID]\SelectedSlot] = 45
	...
	ElseIf mp_I\Gamemode\ID = Gamemode_Deathmatch Then
		...
		If Players[playerID]\WeaponInSlot[SLOT_PRIMARY] <> 0 Then
...
Function ApplyPlayerHitBoxes.HitBox()
	...
	;If gc\CurrGamemode=3 And mp_I\PlayState=GAME_CLIENT Then Return
