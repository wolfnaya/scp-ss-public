		...
		If ens\time <= 0.0 Then
			...
			If EnemyCount <= MaxEnemyCount And EnemyCount2 < mp_I\Gamemode\EnemyCount Then ; ~ Only spawn an enemy if the maximum amount of enemies doesn't get overexceeded
						...
						If Instr(ens\enemies,"zombie")>0 Then
							If (mp_I\BossNPC = Null And (mp_I\Gamemode\Phase / 2) = mp_I\Gamemode\MaxPhase) Then
								Select mp_I\MapInList\BossNPC
									...
									Case "SCP-076-2"
										NPCtoSpawnType = NPCtype076
									Case "SCP-106"
										NPCtoSpawnType = NPCtype106
									Case "SCP-457"
										NPCtoSpawnType = NPCtype457
								End Select
							Else
								NPCtoSpawnType = NPCType049_2MP
							EndIf
							Exit
						EndIf
				...
				Select NPCtoSpawnType
					...
					Case NPCType049_2MP
						If Rand(10)<10 Then
							n = CreateNPC(NPCType049_2MP,EntityX(ens\obj),EntityY(ens\obj)+0.5,EntityZ(ens\obj))
						Else
...
Function UpdateNPCsServer()
	...
	For n = Each NPCs
		...
		Select n\NPCtype
			...
			Case NPCType049_2MP,NPCtypeGuardZombieMP
				UpdateNPCtype049_2MP(n)
		End Select
			...
			Else
							...
							If n2\ID>n\ID Then
								If Abs(DeltaYaw(n\Collider,n2\Collider))<80.0 Then
									If EntityDistanceSquared(n\Collider,n2\Collider)<(n\CollRadius+n2\CollRadius)*0.5 Then							
										TranslateEntity n2\Collider, Cos(EntityYaw(n\Collider,True)+90)* 0.01 * fps\Factor[0], 0, Sin(EntityYaw(n\Collider,True)+90)* 0.01 * fps\Factor[0], True
									EndIf
								EndIf
							Else
								If EntityDistanceSquared(n\Collider,n2\Collider)<(n\CollRadius+n2\CollRadius)*0.5 Then
									TranslateEntity n\Collider, Cos(EntityYaw(n\Collider,True)-45)* 0.01 * fps\Factor[0], 0, Sin(EntityYaw(n\Collider,True)-45)* 0.01 * fps\Factor[0], True
								EndIf
							EndIf
						EndIf
					Next
				...
				If mp_I\PlayState = GAME_SERVER Then
					...
					If n\DeleteTimer <= 0.0 Then
						...
						EndIf
					Else
						n\DeleteTimer = n\DeleteTimer - fps\Factor[0]
					EndIf
				EndIf
			EndIf	
		EndIf
