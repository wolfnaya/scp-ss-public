...
Function MainLoopMenu()
	...
	If MainMenuOpen Then
		...
		EndIf
		If EndingTimer >= 0.0 Then
			Text 20,opt\GraphicHeight-90,"SCP - Security Stories v"+VersionNumber
			Color 131, 187, 239
			Text 20, opt\GraphicHeight-60, GetLocalString("Menu","made_by_nwi")
			Color 255,0,0
			Text 20, opt\GraphicHeight-30, GetLocalString("Menu","by_wolfnaya")
			;! ~ TEST ONLY
			Color 255,255,255
			TextWithAlign 1650,opt\GraphicHeight-30,GetLocalString("Menu","test_ver"),2
			;!------------
			If clm\NTFMode Then
				Color 0,19,200
				TextWithAlign 1650, 0, GetLocalString("Menu","selected_gamemode")+GetLocalString("Menu","gm_ntf"),2
			ElseIf clm\GuardMode Then
				Color 170,170,170
				TextWithAlign 1650, 0, GetLocalString("Menu","selected_gamemode")+GetLocalString("Menu","gm_guard"),2
			ElseIf clm\DMode Then
				Color 255,106,0
				TextWithAlign 1650, 0, GetLocalString("Menu","selected_gamemode")+GetLocalStringR("Menu","gm_d",ClassDNumber),2
			Else
				Color 255,255,255
				TextWithAlign 1650, 0, GetLocalString("Menu","selected_gamemode")+GetLocalString("Menu","gm_r"),2
			EndIf
			
			Color 255,255,255
			TextWithAlign 1650, 20, GetLocalString("Menu","selected_chapter")+Int(cpt\Current),2
			
			Color 255,255,255
			TextWithAlign 1650, 40, GetLocalString("Menu","shop_points")+Int(mnsh\Points),2
			
			If opt\IntroEnabled Then
				Color 255,255,255
				TextWithAlign 1650, 60, GetLocalString("Menu","intro_enabled")+GetLocalString("Menu","True"),2
			Else
				Color 255,255,255
				TextWithAlign 1650, 60, GetLocalString("Menu","intro_enabled")+GetLocalString("Menu","False"),2
			EndIf
...
Function Load3DMenu(customprogress$="")
	...
	LightSpriteTex[0] = LoadTexture_Strict("GFX\Particles\light_1.jpg",1,0)
	LightSpriteTex[2] = LoadTexture_Strict("GFX\Particles\light_sprite.jpg",1,0)
	...
	Select m3d\RoomName
		Case "gate_a_intro"
			...
			;[End Block]
		Case "gate_a_topside","gate_b_topside"
			;[Block]
			m3d\FogColor = FogColor_Outside
		...
		Case "gate_c_topside"
			...
			m3d\Objects[1] = LoadMesh_Strict("GFX\map\Rooms\gate_c_topside\gate_c_topside.b3d")
		...
		Case "gate_a_entrance"
			...
			d_I\DoorOBJ = LoadMesh_Strict("GFX\map\Props\Doors\ElevatorDoor.b3d")
		...
		Case "room1_intro"
			...
			d_I\DoorOBJ = LoadMesh_Strict("GFX\map\Props\Doors\WindowedDoor.b3d")
			HideEntity d_I\DoorOBJ
...
;			HideEntity d_I\DoorOBJ
		...
		Case "checkpoint_acz"
			...
			RotateEntity m3d\Objects[1],0,0,0
			m3d\Objects[2] = LoadMesh_Strict("GFX\Map\Props\Doors\WindowedDoor.b3d")
			ScaleEntity m3d\Objects[2],RoomScale,RoomScale,RoomScale
		...
		Case "surveil_room"
			...
			m3d\Objects[15] = LoadAnimTexture("GFX\HUD\SL_monitors_checkpoint.jpg",1,512,512,0,4)
			m3d\Objects[16] = LoadAnimTexture("GFX\HUD\Sl_monitors.jpg",1,256,256,0,8)
...
Function Update3DMenu()
	...
	Select m3d\RoomName
		...
		Case "room1_intro"
			...
			m3d\State[0] = m3d\State[0] + fps\Factor[0]
...
;			EndIf
		...
		Case "room2_nuke"
			...
			If m3d\State[0]=0 Then
				If m3d\State[1]<1.2 Then
					m3d\State[1]=Min(m3d\State[1]+0.005*fps\Factor[0],1.2)
			...
			Else
				If m3d\State[1]>-2.0 Then
					m3d\State[1]=Max(m3d\State[1]-0.005*fps\Factor[0],-2.0)
		...
		Case "checkpoint_acz"
			...
			If m3d\State[0]=0 Then
				If m3d\State[1]<0.5 Then
					m3d\State[1]=Min(m3d\State[1]+0.0025*fps\Factor[0],0.5)
				Else
					m3d\State[0]=1
				EndIf
			Else
				If m3d\State[1]>0.0 Then
					m3d\State[1]=Max(m3d\State[1]-0.0025*fps\Factor[0],0.0)
				Else
					m3d\State[0]=0
				EndIf
			EndIf
		...
		Case "pocketdimension"
			...
			TurnEntity m3d\Pivot,0,-0.05*fps\Factor[0],0
			...
			Else
				If m3d\State[0]=-1 Then
					If m3d\State[2]>m3d\State[0] Then
						m3d\State[2]=Max(m3d\State[2]-0.01*fps\Factor[0],m3d\State[0])
					Else
						m3d\State[0]=0
					EndIf
				Else
					If m3d\State[2]<m3d\State[0] Then
						m3d\State[2]=Min(m3d\State[2]+0.01*fps\Factor[0],m3d\State[0])
					Else
						m3d\State[0]=0
					EndIf
				EndIf
				...
				If m3d\State[1]=-1 Then
					If m3d\State[3]>m3d\State[1] Then
						m3d\State[3]=Max(m3d\State[3]-0.01*fps\Factor[0],m3d\State[1])
					Else
						m3d\State[1]=0
					EndIf
				Else
					If m3d\State[3]<m3d\State[1] Then
						m3d\State[3]=Min(m3d\State[3]+0.01*fps\Factor[0],m3d\State[1])
					Else
						m3d\State[1]=0
					EndIf
				EndIf
	...
	If m3d\FreeRoam
		...
		MoveEntity m_I\Cam,0.05*((KeyDown(KEY_LEFT)*-1)+(KeyDown(KEY_RIGHT)))*fps\Factor[0],0,0.05*((KeyDown(KEY_UP))+(KeyDown(KEY_DOWN)*-1))*fps\Factor[0]
			...
			If opt\EnableRoomLights%
				...
				If ml\LightConeSpark<>0 
					If ml\LightConeSparkTimer>0 And ml\LightConeSparkTimer<10
						...
						ml\LightConeSparkTimer=ml\LightConeSparkTimer+fps\Factor[0]
					Else
				...
				If (EntityDistanceSquared(cam%,ml\LightSprites2)<PowTwo(8.5)) ;Lor ml\RoomTemplate\UseLightCones)
					If EntityVisible(cam%,ml\LightSpritesPivot) ;Lor ml\RoomTemplate\UseLightCones
						...
						;			ShowEntity ml\LightConeSpark
						;			ml\LightConeSparkTimer = fps\Factor[0]
						;		EndIf
						;	EndIf
						;EndIf
...
Function ReloadAll()
	...
	Next
	FreeImage NavBG
	FreeImage MenuMeterIMG
	For i = 0 To 1
		FreeImage PauseMenuIMG[i]
	Next
	For i = 0 To 6
		FreeImage PlayerIcons[i]
	Next
	FreeImage Panel294
	
	FreeImage FineKevlarIcon
	
	For i = 0 To 15
		FreeImage AttachmentIMG[i]
	Next
	For i = 0 To 12
		FreeImage GunAttachmentPreviewIMG[i]
	Next
	For i = 0 To 3
		FreeImage SCPIcons[i]
	Next
	For i = 0 To MAXACHIEVEMENTS-1
		FreeImage achv\AchvIMG[i]
	Next
	FreeImage achv\AchvLocked
	...
	For i = 0 To 1
		MaskImage PauseMenuIMG[i], 255,255,0
