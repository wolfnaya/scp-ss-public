...
Global ParticleTextures%[22] ; TODO this can be shrunk
...
Function UpdateParticles()
	...
	For p.Particles = Each Particles
		...
		MoveEntity(p\pvt, 0, 0, p\speed * fps\Factor[0])
		If p\gravity <> 0 Then p\yspeed = p\yspeed - p\gravity * fps\Factor[0]
		TranslateEntity(p\pvt, 0, p\yspeed * fps\Factor[0], 0, True)
...
Global WaterHissSFX% = LoadSound_Strict("SFX\General\Water_Hiss.ogg")
...
Function UpdateEmitters()
	...
	For e.Emitters = Each Emitters
		...
		EndIf
		If fps\Factor[0] > 0 And updateEmitter Then
			If e\emittertype=2
			...
			If e\emittertype = 5 Then
				If e\Emitterlifetime <> 0 Then
					e\Emitterlifetime = e\Emitterlifetime-fps\Factor[0]
			...
			If InSmoke = False Then
				If e\emittertype<> 4 Then
					If (Not mpl\HasNTFGasmask) And (Not wbi\GasMask) And (Not hds\Wearing) And (Not wbi\Hazmat) Then
	...
	If InSmoke Then
		...
		EyeIrritation = Min(EyeIrritation + fps\Factor[0] * 4.0, 25 * 70)
	Else
		If BlinkTimer < 0 Then
			EyeIrritation = Max(0.0, EyeIrritation - fps\Factor[0] * 4.0)
		Else
			EyeIrritation = Max(0.0, EyeIrritation - fps\Factor[0] * 2.0)
		EndIf
	EndIf
