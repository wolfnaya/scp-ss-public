		...
		For i = 0 To 31
			If snd\channels[i] <> 0 Then
				If Not ChannelPlaying(snd\channels[i]) Then
					...
					;	If snd\HasSubtitles Then ShowSubtitles(snd\name)
					;EndIf
...
; ~ Hide and ShowEntity_Strict made by Wolfnaya, even if names are absolutely ridiculous...

Function HideEntity_Strict%(entity%,alpha#=0)
	
	If (Not EntityHidden(entity)) Then
		HideEntity entity
...
Function ShowEntity_Strict%(entity%,alpha#=1)
	
	If EntityHidden(entity) Then
		ShowEntity entity
		EntityAlpha entity, alpha
	EndIf
