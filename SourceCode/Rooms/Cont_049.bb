...
Function FillRoom_Cont_049(r.Rooms)
	...
	d.Doors = CreateDoor(0, r\x,0,r\z, 0, r, False, 2, SEVERED_HAND_2)
		...
		Else
			...
			ElseIf e\EventState[0] > 0
				...
				If e\EventState[0] >= 70 Then
					...
					EndIf
				Else
					e\EventState[0] = Min(e\EventState[0]+fps\Factor[0],70)
				EndIf
				...
				ElseIf e\EventState[0] < 70*240
					For n.NPCs = Each NPCs ;awake the zombies
						If n\NPCtype = NPCtype049_2 And n\State = 0 Then
							n\State = 1
							SetNPCFrame(n, 155)
						EndIf
					Next
