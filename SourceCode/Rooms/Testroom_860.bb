...
Function FillRoom_Testroom_860(r.Rooms)
	...
	d = CreateDoor(r\zone, r\x+416.0*RoomScale,0,r\z - 640.0 * RoomScale,0,r,False,False,KEY_CARD_1)
	d = CreateDoor(r\zone, r\x+416.0*RoomScale,0,r\z + 640.0 * RoomScale,0,r,False,False,KEY_CARD_1)
...
Global Is860_Chasing% = False
Global IsInsideForest% = False
		...
		If e\EventState[0]=1.0 Then ;the player is in the forest
			...
			If e\room\NPC[0]<>Null Then
				If (e\room\NPC[0]\State2 = 1 And e\room\NPC[0]\State>1) Lor e\room\NPC[0]\State>2 ;the monster is chasing the player
					Is860_Chasing = True
					ShouldPlay = MUS_CHASE_860
				Else
					Is860_Chasing = False
					ShouldPlay = MUS_CONT_860
				EndIf
			EndIf
			...
			If (Not NoClip)
				...
				ElseIf EntityY(Collider)>EntityY(fr\Forest_Pivot,True)+0.5
					MoveEntity(Collider, 0, ((EntityY(fr\Forest_Pivot,True)+0.5) - EntityY(Collider))*fps\Factor[0], 0)
				EndIf
			EndIf
			
			If e\room\NPC[0]<>Null
				If e\room\NPC[0]\State = 0 Lor EntityDistanceSquared(Collider, e\room\NPC[0]\Collider)>PowTwo(20.0) Then
					e\EventState[2] = e\EventState[2] + (1+CurrSpeed)* fps\Factor[0]
					If (e\EventState[2] Mod 500) < 10.0 And ((e\EventState[2]-fps\Factor[0]) Mod 500) > 490.0 Then
				...
				If (DistanceSquared(EntityX(e\room\Objects[3],True),EntityX(Collider,True),EntityZ(e\room\Objects[3],True),EntityZ(Collider,True))<1.0) Then
					...
					If SelectedItem = Null Then
						If KeyHitUse Then
							...
							CreateMsg(GetLocalString("Singleplayer", "testroom_860_1"))
						EndIf
					ElseIf SelectedItem\itemtemplate\tempname="scp860" Lor SelectedItem\itemtemplate\tempname="scp005" 
						If KeyHitUse Then
							If SelectedItem\itemtemplate\tempname="scp005" Then
								CreateMsg(GetLocalString("Items","scp005_8"))
							EndIf
							PlaySound_Strict(LoadTempSound("SFX\Door\WoodenDoorOpen.ogg"))
