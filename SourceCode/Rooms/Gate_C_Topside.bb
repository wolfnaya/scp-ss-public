...
Function FillRoom_Gate_C_Topside(r.Rooms)
	...
	it = CreateItem("AK-12","ak12",r\x+4975*RoomScale,r\y+1682*RoomScale,r\z+1830*RoomScale)
	it\state = 30 : it\state2 = 210
	EntityParent it\collider, r\obj
	
	it = CreateItem("Heavy Ballistic Vest","finevest",r\x+4975*RoomScale,r\y+1682*RoomScale,r\z+1830*RoomScale)
	EntityParent it\collider, r\obj
	
	r\Objects[0] = LoadMesh_Strict("GFX\map\rooms\gate_c_topside\gate_c_topside.b3d",r\obj)
	...
	MeshCullBox (r\Objects[2], -MeshWidth(r\Objects[2]), -MeshHeight(r\Objects[2]), -MeshDepth(r\Objects[2]), MeshWidth(r\Objects[2])*2, MeshHeight(r\Objects[2])*2, MeshDepth(r\Objects[2])*2)
	...
	r\Objects[9] = LoadRMesh("GFX\map\Elevators\elevator.rmesh",Null)
	r\Objects[10] = CopyEntity(r\Objects[9],r\obj)
	...
	If PlayerRoom = e\room Then
		...
		If e\EventState[0] = 0 Then
			If HUDenabled And psp\IsShowingHUD Then
				CreateSplashText(GetLocalString("Singleplayer","chapter_9"),opt\GraphicWidth/2,opt\GraphicHeight/2,100,5,Font_Default_Large,True,False)
			EndIf
			AddShopPoints(200)
		EndIf
		...
		If e\EventState[0] = 0 Then
			...
			DrawLoading(100, False, "", "LOADING DONE")
		...
		Else
					...
					If e\EventState[2] < 70*70 Then
						...
						If ChannelPlaying(e\SoundCHN2)=False Then
							If opt\MusicVol > 0 Then
								e\SoundCHN2 = PlaySound_Strict(LoadTempSound("SFX\Music\Misc\Rocket_Launch_Success.ogg"))
							EndIf
						EndIf
					Else
						ShouldPlay = MUS_NULL
			...
			If ecst\SuccessRocketLaunch Then
				...
				If e\EventState[2] >= 70*35 And e\EventState[2] < 70*50 Then
					...
					MoveEntity(e\room\Objects[3],0,0.26,0)
					MoveEntity(e\room\Objects[4],0,0.26,0)
					MoveEntity(e\room\Objects[5],0,0.26,0)
					MoveEntity(e\room\Objects[6],0,0.26,0)
			...
			If (Not ecst\SuccessRocketLaunch) Then
				...
				If e\EventState[2] >= 70*23.5 And e\EventState[2] < 70*23.525 Then
					...
					If ChannelPlaying(e\SoundCHN)=False Then
						e\SoundCHN = PlaySound_Strict(LoadTempSound("SFX\Ending\GateC\Rocket_Explosion.ogg"))
					EndIf
					If ChannelPlaying(e\SoundCHN2)=False Then
						If opt\MusicVol > 0 Then
							e\SoundCHN2 = PlaySound_Strict(LoadTempSound("SFX\Music\Misc\Rocket_Launch_Fail.ogg"))
						EndIf
					EndIf
				...
				If e\EventState[2] >= 70*23.5 And e\EventState[2] < 70*28.5 Then
					...
					If EntityVisible(Camera, e\room\Objects[2]) Then
						CreateExplosionWaveParticle(EntityX(rocketbone2,True),EntityY(rocketbone2,True) +1.5,EntityZ(rocketbone2,True))
					EndIf
					p.Particles = CreateParticle(EntityX(rocketbone2, True), EntityY(rocketbone2, True) +1.5, EntityZ(rocketbone2, True), 16, 2.5 ,0, 25)
			...
			If e\EventState[3] = 1 Then
				...
				If e\room\NPC[0] = Null Then
					e\room\NPC[0]=CreateNPC(NPCtypeCI, EntityX(e\room\Objects[11],True), EntityY(e\room\Objects[11],True), EntityZ(e\room\Objects[11],True))
					RotateEntity e\room\NPC[0]\Collider, 0, EntityYaw(e\room\obj)+90,0, True
				EndIf
				If e\room\NPC[1] = Null Then
					e\room\NPC[1]=CreateNPC(NPCtypeCI, EntityX(e\room\Objects[11],True), EntityY(e\room\Objects[11],True), EntityZ(e\room\Objects[11],True)+0.5)
					RotateEntity e\room\NPC[1]\Collider, 0, EntityYaw(e\room\obj)+90,0, True
				EndIf
				If e\room\NPC[2] = Null Then
					e\room\NPC[2]=CreateNPC(NPCtypeCI, EntityX(e\room\Objects[11],True), EntityY(e\room\Objects[11],True), EntityZ(e\room\Objects[11],True)+0.7)
					RotateEntity e\room\NPC[2]\Collider, 0, EntityYaw(e\room\obj)+90,0, True
				EndIf
				If e\room\NPC[3] = Null Then
					e\room\NPC[3]=CreateNPC(NPCtypeCI, EntityX(e\room\Objects[13],True), EntityY(e\room\Objects[13],True), EntityZ(e\room\Objects[13],True))
					RotateEntity e\room\NPC[3]\Collider, 0, EntityYaw(e\room\obj)+90,0, True
				EndIf
				If e\room\NPC[4] = Null Then
					e\room\NPC[4]=CreateNPC(NPCtypeCI, EntityX(e\room\Objects[13],True), EntityY(e\room\Objects[13],True), EntityZ(e\room\Objects[13],True)+0.5)
					RotateEntity e\room\NPC[4]\Collider, 0, EntityYaw(e\room\obj)+90,0, True
				EndIf
				If e\room\NPC[5] = Null Then
					e\room\NPC[5]=CreateNPC(NPCtypeCI, EntityX(e\room\Objects[13],True), EntityY(e\room\Objects[13],True), EntityZ(e\room\Objects[13],True)+0.7)
				...
				If DistanceSquared(EntityX(Collider),EntityX(e\room\Objects[1],True),EntityZ(Collider),EntityZ(e\room\Objects[1],True)) < PowTwo(0.8) Then
					If e\EventState[1] <> 1.0 Then
						...
						If KeyHitUse Then
							e\EventState[1] = 1.0
							e\EventState[2] = 70*1
							If (Not ChannelPlaying(e\SoundCHN4))Then
								e\SoundCHN4 = PlaySound_Strict(LoadTempSound("SFX\Ending\GateC\Rocket_Moving.ogg"))
							EndIf
							If ecst\SuccessRocketLaunch = True Then
								...
								If (Not ChannelPlaying(e\SoundCHN)) Then
									e\SoundCHN = PlaySound_Strict(LoadTempSound("SFX\Ending\GateC\Rocket_Launch.ogg"))
								EndIf
							Else
								If (Not ChannelPlaying(e\SoundCHN3)) Then
									e\SoundCHN3 = PlaySound_Strict(LoadTempSound("SFX\Ending\GateC\Rocket_Failure.ogg"))
								EndIf
							EndIf
						EndIf
					EndIf
