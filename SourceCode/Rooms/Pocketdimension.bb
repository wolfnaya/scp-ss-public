...
;	Next
	...
	r\Objects[18] = LoadTexture_Strict("GFX\Npcs\SCPs\106\pd_plane.png",1+2,2)
	;HideEntity(r\Objects[18])
	r\Objects[19] = LoadTexture_Strict("GFX\Npcs\SCPs\106\pd_plane_2.png",1+2,2)
...
Function UpdateEvent_Pocketdimension(e.Events)
	...
	If PlayerRoom = e\room Then
		...
		For i2 = 8 To 11
			ShowEntity(e\room\Objects[i2])
		Next
;		For i3 = 17 To 19
;			ShowEntity(e\room\Objects[i3])
;		Next
		...
		If e\EventState[1] = 1 Then ;in the second room
			...
			Else
				...
				If EntityY(Collider)>6.0 Then
					...
					MoveEntity(Collider, 0, Min((12.0 - EntityY(Collider)),0.0)*fps\Factor[0], 0)
					...
					ElseIf dist < 8.0
						...
						;Injuries=Injuries+(8.0-dist)*fps\Factor[0]*0.0005
				...
				Else
					...
					If temp < 2000*RoomScale Then
						;Injuries = Injuries + (fps\Factor[0]/4000)
						
;						If Injuries > 1.0 Then
;							If Injuries - (fps\Factor[0]/4000)=< 1.0 Then
;								PlaySound_Strict LoadTempSound("SFX\Room\PocketDimension\Kneel.ogg")
;							EndIf
;						EndIf
						
						Sanity = Max(Sanity - fps\Factor[0] / temp / 8,-1000)
		...
		Else ;pillar room
			...
			If ParticleAmount > 0
				If Rand(800)=1 Then 
					...
					p.Particles = CreateParticle(EntityX(Collider)+Cos(angle)*7.5, 0.0, EntityZ(Collider)+Sin(angle)*7.5, 17, 4.0, 0.0, 2500)
					EntityBlend(p\obj, 2)
					p\speed = 0.01
					p\SizeChange = 0
...
Function TeleportPlayerFromPD(e.Events)
	...
	Select gc\CurrZone
		...
		Case LCZ
			For e2.Events = Each Events
				If e2\EventName = "cont_914_372" Then
					temp = True
					Exit
				EndIf
			Next
		...
		Case RCZ
			For e2.Events = Each Events
				If e2\EventName = "checkpoint_rcz" Then
					temp = True
					Exit
				EndIf
			Next
	...
	For r.Rooms = Each Rooms
		If gc\CurrZone = HCZ Then
			If r\RoomTemplate\Name = "room1_sewers" And temp Then
				...
				Exit
			ElseIf r\RoomTemplate\Name = "core_hcz" And random <= 3 And (Not temp) Then
				TeleportEntity(Collider,EntityX(r\obj,True),0.4,EntityZ(r\obj,True),0.3,True)
				Exit
			ElseIf r\RoomTemplate\Name = "facility_reactor_entrance" And (random > 3 And random <= 6) And (Not temp) Then
				TeleportEntity(Collider,EntityX(r\Objects[10],True),0.4,EntityZ(r\Objects[10],True),0.3,True)
				Exit
			ElseIf r\RoomTemplate\Name = "checkpoint_hcz" And random > 6 And (Not temp) Then	
				TeleportEntity(Collider,EntityX(r\Objects[0],True),0.4,EntityZ(r\Objects[0],True),0.3,True)
				Exit
			EndIf
		ElseIf gc\CurrZone = LCZ Then
			If r\RoomTemplate\Name = "cont_914_372" And temp Then
				TeleportEntity(Collider,EntityX(r\obj,True),0.4,EntityZ(r\obj,True),0.3,True)
				Exit
			ElseIf r\RoomTemplate\Name = "room2_3" And random <= 3 Then
				TeleportEntity(Collider,EntityX(r\obj,True),0.4,EntityZ(r\obj,True),0.3,True)
				Exit
			...
			ElseIf r\RoomTemplate\Name = "room2c_1" And random > 6 Then
				...
				Exit
			EndIf
		ElseIf gc\CurrZone = EZ Then
			If r\RoomTemplate\Name = "room2_ws" And temp Then
				TeleportEntity(Collider,EntityX(r\obj,True),0.4,EntityZ(r\obj,True),0.3,True)
				Exit
			ElseIf r\RoomTemplate\Name = "room4_ez" And random <= 3 Then
				TeleportEntity(Collider,EntityX(r\obj,True),0.4,EntityZ(r\obj,True),0.3,True)
				Exit
			ElseIf r\RoomTemplate\Name = "endroom_1" And (random > 3 And random <= 6) Then
				TeleportEntity(Collider,EntityX(r\obj,True),0.4,EntityZ(r\obj,True),0.3,True)
			...
			ElseIf r\RoomTemplate\Name = "room2c_ez" And random > 6 Then
				...
				Exit
			EndIf
		ElseIf gc\CurrZone = RCZ Then
			If r\RoomTemplate\Name = "checkpoint_rcz" Then
				TeleportEntity(Collider,EntityX(r\obj,True),0.4,EntityZ(r\obj,True),0.3,True)
				Exit
			EndIf
		ElseIf gc\CurrZone = BCZ Then
			If r\RoomTemplate\Name = "cont_409" Then
				TeleportEntity(Collider,EntityX(r\obj,True),0.4,EntityZ(r\obj,True),0.3,True)
				Exit
			EndIf
		EndIf	
