...
Function UpdateEvent_Area_076_Entrance(e.Events)
	...
	If e\room\NPC[0] = Null Then
		e\room\NPC[0] = CreateNPC(NPCTypeWoundedGuard,EntityX(e\room\Objects[0],True),EntityY(e\room\Objects[0],True)+0.2,EntityZ(e\room\Objects[0],True))
			...
			If (I_268\Using = 0 Lor I_268\Timer =< 0.0) Then
				If (Not ecst\WasIn076) Then
					...
					If e\EventState[0] > 70*12 Then
						e\EventState[1] = e\EventState[1] + (0.01*fps\Factor[0])
...
Function FillRoom_Area_076(r.Rooms)
	...
	it = CreateItem("H&K 416", "hk416", r\x -494.0 * RoomScale, r\y +140.0 * RoomScale, r\z +3418.0 * RoomScale)
	EntityParent(it\collider, r\obj)
	it\state2 = 60 : it\state = 30
	
	it = CreateItem("Laser Sight", "lasersight", r\x -867.0 * RoomScale, r\y +130.0 * RoomScale, r\z +3086.0 * RoomScale)
	EntityParent(it\collider, r\obj)
	...
	it = CreateItem("H&K MP5K", "mp5k", r\x +541.0 * RoomScale, r\y +210.0 * RoomScale, r\z +2653.0 * RoomScale)
	EntityParent(it\collider, r\obj)
	it\state2 = 90  : it\state = 30
	
	it = CreateItem("Acog Scope", "acog", r\x -548.0 * RoomScale, r\y +155.0 * RoomScale, r\z +2609.0 * RoomScale)
	EntityParent(it\collider, r\obj)
	
	it = CreateItem("Laboratory document", "paper", r\x +1883.0 * RoomScale, r\y -2150.0 * RoomScale, r\z +4690.0 * RoomScale)
	EntityParent(it\collider, r\obj)
	
	it = CreateItem("Storage Area Access Cave Card", "key_cave2", r\x +2309.0 * RoomScale, r\y -2011.0 * RoomScale, r\z +5960.0 * RoomScale)
	EntityParent(it\collider, r\obj)
	...
	If PlayerRoom = e\room
		...
		If e\EventState[3] = 0 Then
			...
			If e\room\NPC[1] = Null Then
				e\room\NPC[1]=CreateNPC(NPCTypeWoundedGuard,EntityX(e\room\Objects[6],True),EntityY(e\room\Objects[6],True)+0.5,EntityZ(e\room\Objects[6],True))
				e\room\NPC[1]\State = 2
				RotateEntity(e\room\NPC[1]\Collider,0,e\room\angle-115,0)
				
				If HUDenabled And psp\IsShowingHUD Then
					CreateSplashText(GetLocalString("Singleplayer","chapter_6"),opt\GraphicWidth/2,opt\GraphicHeight/2,100,5,Font_Default_Large,True,False)
				EndIf
				AddShopPoints(200)
				cpt\Unlocked = 6
			EndIf
		...
		If e\EventState[3] = 1 Then
			e\room\NPC[0]=CreateNPC(NPCtype076,EntityX(e\room\Objects[9],True),EntityY(e\room\Objects[9],True)+0.5,EntityZ(e\room\Objects[9],True))
			e\room\NPC[0]\State = SCP076_EVENT_WANDER
		...
		If e\EventState[3] = 4 Then
			...
			e\EventState[4] = e\EventState[4] + (0.01*fps\Factor[0])
