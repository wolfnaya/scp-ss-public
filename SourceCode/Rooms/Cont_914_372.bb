...
Function FillRoom_Cont_914_372(r.Rooms)
	...
	rt\objPath = "GFX\map\rooms\cont_914_372\cont_914_372_down.rmesh"
	...
	rt\objPath = "GFX\map\rooms\cont_914_372\cont_914_372_down_2.rmesh"
	rt\obj = LoadRMesh(rt\objPath,rt)
	r\Objects[5] = rt\obj
	...
	r\Objects[0] = LoadMesh_Strict("GFX\map\rooms\cont_914_372\914_key.b3d")
	r\Objects[1] = LoadMesh_Strict("GFX\map\rooms\cont_914_372\914_knob.b3d")
	r\Objects[9] = LoadAnimMesh_Strict("GFX\map\rooms\cont_914_372\cont_914_372_down_blinds.b3d")
	...
	r\Levers[0] = CreateLever(r, r\x +1484.0 * RoomScale, r\y -6594.0 * RoomScale, r\z +2514 * RoomScale,180,True)
	r\Levers[1] = CreateLever(r, r\x +1771.0 * RoomScale, r\y -6594.0 * RoomScale, r\z +2514 * RoomScale,180,True)
	...
	it = CreateItem("Scope Battery", "scopebat", r\x +1600.0 * RoomScale, r\y -6550.0 * RoomScale, r\z + 3686.0 * RoomScale)
	EntityParent(it\Collider, r\obj)
	
	it = CreateItem("First Aid Kit", "firstaid", r\x + 1607.0 * RoomScale, r\y -6640.0 * RoomScale, r\z + 3597.0 * RoomScale)
	...
	it = CreateItem("Radio Transceiver", "radio", r\x -522.0 * RoomScale, r\y -9620.0 * RoomScale, r\z -235.0 * RoomScale)
	it\state = 80.0
	EntityParent(it\collider, r\obj)
...
Function UpdateEvent_Cont_914_372(e.Events)
		...
		If e\EventState[0] > 0 Then
			...
			If DistanceSquared(EntityX(Collider), EntityX(e\room\Objects[2], True), EntityZ(Collider), EntityZ(e\room\Objects[2], True)) < PowTwo(170.0 * RoomScale) Then
				...
				If setting = ROUGH Lor setting = COARSE Then
					If e\EventState[0] > 70 * 2.6 And e\EventState[0] - fps\Factor[1] < 70 * 2.6 Then PlaySound_Strict Death914SFX
				EndIf
				...
				If e\EventState[0] > 70 * 3 Then
					Select setting
						...
						Case COARSE
							BlinkTimer = -10
							If e\EventState[0] - fps\Factor[1] < 70 * 3 Then PlaySound_Strict Use914SFX
						Case ONETOONE
							BlinkTimer = -10
							If e\EventState[0] - fps\Factor[1] < 70 * 3 Then PlaySound_Strict Use914SFX
						Case FINE, VERY_FINE
	...
	If PlayerRoom = e\room Then
		If e\EventState[3] = 0 Then
			If EntityY(Collider)<-7500.0*RoomScale And (Not PlayerInNewElevator) Then
				...
				CreateNPC(NPCtype372, -933*RoomScale, -9700*RoomScale, -187*RoomScale)
				e\EventState[3] = 1
				;RemoveEvent(e)
			EndIf					
		EndIf
