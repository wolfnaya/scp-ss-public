...
Function FillRoom_Room1_Start(r.Rooms)
	...
	r\RoomDoors[10] = CreateDoor(r\zone,r\x-133.0*RoomScale,r\y-2746*RoomScale,r\z+106.0*RoomScale,90,r,False,DOOR_CONTAINMENT, KEY_CARD_1)
	...
	Select Rand(0,2)
		Case 0
			it = CreateItem("Silo Door Cave Card", "key_cave", r\x-3388.0*RoomScale, r\y-3342.0*RoomScale, r\z+4306.0*RoomScale)
			EntityParent(it\collider, r\Objects[10])
		Case 1
			it = CreateItem("Silo Door Cave Card", "key_cave", r\x-11730.0*RoomScale, r\y-3326.0*RoomScale, r\z+2124.0*RoomScale)
			EntityParent(it\collider, r\Objects[10])
		Case 2
			it = CreateItem("Silo Door Cave Card", "key_cave", r\x-9014.0*RoomScale, r\y-2796.0*RoomScale, r\z+7780.0*RoomScale)
			EntityParent(it\collider, r\Objects[10])
	End Select
	
	it = CreateItem("Storage Area Access Cave Card", "key_cave2", r\x-5259.0*RoomScale, r\y-1425.0*RoomScale, r\z+7750.0*RoomScale)
	EntityParent(it\collider, r\Objects[10])
	
	it = CreateItem("O5 Council Room Note", "paper", r\x-6472.0*RoomScale, r\y-1296.0*RoomScale, r\z+8760.0*RoomScale)
	EntityParent(it\collider, r\Objects[10])
	
	it = CreateItem("Level 1 Key Card", "key1", r\x+255.0*RoomScale, r\y-2736.0*RoomScale, r\z+98.0*RoomScale)
	EntityParent(it\collider, r\Objects[11])
	
	If (Not clm\GlobalMode) Lor cpt\Current = 1 Then
		it = CreateItem("Crowbar", "crowbar", r\x+276.0*RoomScale, r\y+66.0*RoomScale, r\z-622.0*RoomScale)
		EntityParent(it\collider, r\Objects[11])
	EndIf
		...
		If PlayerRoom = e\room Then
			...
			If (Not ecst\NewCavesEvent) Then
				...
				If e\EventState[7] = 1 Then
					e\EventState[8] = e\EventState[8] + (fps\Factor[0]*0.01)
				...
				If EntityDistanceSquared(e\room\RoomDoors[6]\buttons[0], Camera) < PowTwo(1.0) Then
					If e\EventState[11] = 0 Then
						If e\room\NPC[0] = Null Then
							e\room\NPC[0]=CreateNPC(NPCtype008_4, EntityX(e\room\Objects[12],True),EntityY(e\room\Objects[12],True), EntityZ(e\room\Objects[12],True))
							e\room\NPC[0]\State = 2
							If opt\MusicVol > 0 Then
								PlaySound_Strict(LoadTempSound("SFX\Music\Misc\Unpleasant_Feeling.ogg"))
								EndIf
							PlayNewDialogue(5,%01)
						EndIf
						e\EventState[11] = 1
					EndIf
				EndIf
				...
				If EntityDistanceSquared(e\room\Objects[15], Collider) < PowTwo(1.0) Then
					If e\room\NPC[1] = Null Then
						e\room\NPC[1]=CreateNPC(NPCtypeD, EntityX(e\room\Objects[13],True),EntityY(e\room\Objects[13],True), EntityZ(e\room\Objects[13],True))
				...
				Select e\EventState[0]
					Case 0
						If opt\IntroEnabled Then
							CreateSplashText(GetLocalString("Singleplayer","5_hours_later"),opt\GraphicWidth/2,opt\GraphicHeight/2,280,1,Font_Default_Large,True);,"SFX\Interact\Typing.ogg")
						Else
							CreateSplashText(GetLocalString("Singleplayer","5_hours_after_breach"),opt\GraphicWidth/2,opt\GraphicHeight/2,280,1,Font_Default_Large,True);,"SFX\Interact\Typing.ogg")
						EndIf
						BlurTimer = 0
						BlinkTimer = -10.0
						...
						For i = 0 To MaxItemAmount-1
							If Inventory[i] <> Null Then
								If Inventory[i]\itemtemplate\tempname = "usp" Then
						...
						For i = 0 To MaxItemAmount-1
							If Inventory[i] <> Null Then
								If Inventory[i]\itemtemplate\tempname = "helmet" Then
									RemoveItem(Inventory[i])
									Exit
								EndIf
							EndIf
						...
						For i = 0 To MaxItemAmount-1
							If Inventory[i] <> Null Then
								If Inventory[i]\itemtemplate\tempname = "key2" Then
									RemoveItem(Inventory[i])
									Exit
								EndIf
							EndIf
				...
				If e\EventState[0] >= 2.0 And e\EventState[0] < 3.0 Then
					...
					PositionEntity Collider, EntityX(e\room\Objects[1], True), EntityY(e\room\Objects[1], True), EntityZ(e\room\Objects[1], True)
					ResetEntity Collider
					e\EventState[0] = e\EventState[0] + (fps\Factor[0] / 70.0)/10
					User_Camera_Pitch = -89
				...
				ElseIf e\EventState[0] >= 3.0 And e\EventState[0] < 5.0 Then
					...
					If e\EventState[0] >= 4.0 Then
						...
						If e\EventState[0] >= 5.0 Then
							...
							e\EventState[0] = 6
							PlayNewDialogue(2,%01)
							If HUDenabled And psp\IsShowingHUD Then
								CreateSplashText(GetLocalString("Singleplayer","chapter_1"),opt\GraphicWidth/2,opt\GraphicHeight/2,100,5,Font_Default_Large,True,False)
							EndIf
						EndIf
					EndIf
				EndIf
			...
			Else
				...
				If e\EventState[9] = 1 Then
					e\EventState[4] = e\EventState[4] + (fps\Factor[0]*0.01)
			...
			; ~ New Event
				...
				If (Not TaskExists(TASK_SEARCH_CAVES)) Then
					If e\EventState[6] = 0 Then
						If HUDenabled And psp\IsShowingHUD Then
							CreateSplashText(GetLocalString("Singleplayer","chapter_7"),opt\GraphicWidth/2,opt\GraphicHeight/2,100,5,Font_Default_Large,True,False)
						EndIf
						AddShopPoints(200)
						cpt\Unlocked = 7
