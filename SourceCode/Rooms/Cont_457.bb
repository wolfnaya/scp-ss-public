		...
		If EntityY(Collider)>4500.0*RoomScale ;Level 3
			...
			If (Not PlayerInNewElevator)
				...
				If e\room\NPC[0] = Null Then
					e\room\NPC[0] = CreateNPC(NPCtype457,EntityX(e\room\Objects[2],True),EntityY(e\room\Objects[2],True),EntityZ(e\room\Objects[2],True))
				EndIf
				If e\room\NPC[1] = Null Then
					e\room\NPC[1] = CreateNPC(NPCtypeD,EntityX(e\room\Objects[4],True),EntityY(e\room\Objects[4],True),EntityZ(e\room\Objects[4],True))
					...
					e\room\NPC[1]\texture = "GFX\Npcs\Humans\Personnel\457victim.jpg"
					...
					Local it.Items = CreateItem("Level 4 Key Card", "key4",EntityX(bone%,True),EntityY(bone%,True),EntityZ(bone%,True))
...
;							ne\floorlocked[i] = True ; ~ Locking the elevators untill SCP-457 is contained.
;						Else
;							ne\floorlocked[i] = False ; ~ Unlocking the elevators when SCP-457 is contained.
;						EndIf
;					Next
;					ne\floorlocked[1] = True
;				Next
				...
				If e\room\NPC[0] <> Null Then
					...
					If e\EventState[1] = 0 Then
						If EntityDistanceSquared(e\room\NPC[0]\Collider,e\room\Objects[3]) < PowTwo(0.6) Then
							e\room\NPC[0]\State = SCP457_STATE_STUNNED
							e\EventState[1] = 1
						EndIf
					Else
